...
def create_employee(employee: EmployeeCreate, db: Session = Depends(get_db), user = Depends(has_role(["admin"]))):
...
def update_employee(employee_id: str, employee: EmployeeUpdate, db: Session = Depends(get_db), user = Depends(has_role(["admin"]))):
    ...
    if employee.role_id is not None:
        db_employee.role_id = employee.role_id  # legacy
